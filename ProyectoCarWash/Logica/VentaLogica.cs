    ...
    public class VentaLogica
    {
        ...
        public List<Venta> Listar()
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("select IdVenta,IdRecepcionLavado,Total,Estado from venta", oConexion);
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            Lista.Add(new Venta()
                            {
                                ...
                                oRecepcion = new RecepcionLavado() { IdRecepcionLavado = Convert.ToInt32(dr["IdRecepcionLavado"]) },
        ...
        public bool Registrar(Venta objeto)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    ...
                    sb.AppendLine(string.Format("insert into VENTA(IdRecepcionLavado,Total,Estado) values ({0},{1},'{2}')", objeto.oRecepcion.IdRecepcionLavado,objeto.Total,objeto.Estado));
